...
def make_densebox_target(gt_boxes: np.array) -> Tuple:
    ...
    y_coords, x_coords = torch.meshgrid(x_coords, y_coords)  # (H, W)生成二维网格
    ...
    y_coords_on_fm,
    x_coords_on_fm,
    :] = is_in_boxes  # & is_in_layer  # xy[:, 0], xy[:, 1] reduce dim by 1 to match is_in_boxes.shape & is_in_layer.shape
...
if __name__ == '__main__':
    # gt_boxes
    gt_boxes = np.asarray([[ 85, 100, 125,120]])
    ...
    target = make_densebox_target(gt_boxes)
