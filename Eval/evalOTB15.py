...
def overlap_ratio(rect1, rect2):
    ...
    - rect: 1d array of [x,y,w,h] or
            2d array of N x [x,y,w,h]左上角坐标和目标的W、H
    '''
...
def eval_auc(dataset='OTB2015', tracker_reg='Siam3D_OTB2015', start=0, end=1e6):
    list_path = os.path.join('/home/guiyan/workspaces/liangmin/Siam3DM1/Track/data/', dataset + '.json')
    ...
    trackers = glob.glob(fullfile('/home/guiyan/workspaces/liangmin/Siam3DM1/Track/test', dataset, tracker_reg))
    ...
    for i in range(n_seq):
        seq = seqs[i]
        gt_rect = np.array(annos[seq]['gt_rect']).astype(np.float)#左上角和W、H
        gt_center = convert_bb_to_center(gt_rect)#中心点和W、H
        for j in range(len(trackers)):
            tracker = trackers[j]
            print('{:d} processing:{} tracker: {}'.format(i, seq, tracker))
            bb = get_result_bb(tracker, seq)#左上角和W、H
