...
def track_video(model, video):
    ...
    # # image_files, gt = video['img_names'], video['gt_rect']
    # gt=np.array(gt)
    # print(gt)
    ...
    for f in range(0,len(image_files),1):
        ...
        if f == 0:  # init
            ...
            # regions.append(gt_bbox_)
            regions.append(1)

        elif f > 0 :  # tracking
            gt[f]=np.array(get_axis_aligned_bbox(np.array(gt[f])))
            # print(gt[f])
            gt[f]=cxy_wh_2_rect2(gt[f])
            for num in range(3):
                if f+num >= len(image_files)-1:
                    imgindex=f
                else:
                    imgindex=f+num
                # print(imgindex)
                ims.append(image_files[imgindex])
                rect.append(gt[f])
             
       
            state,pre,Alliou = Siam3D_track(state, ims,model,regions,rect,Alliou,f)  # track
            if overlap_ratio(gt[f],pre)==0:
                lost_number += 1
            
            ims = []
            rect=[]
            # Alliou.append(overlap_ratio(rect2_wh,np.array(pre)))
         
            
            # print(f,gt[f],pre,overlap_ratio(gt[f],np.array(pre)))
    ...
    print("Alliou:",np.mean(Alliou),len(Alliou))
    # print(toc)
...
def main():
    ...
    checkpoint=torch.load('/home/guiyan/workspaces/liangmin/Siam3DM1/model3/1106model_best.pth',map_location='cuda')
    ...
    # model.to(device)
    # model.eval()   
    # net =SiamR3D(branch=R3DNet((2, 2, 2,2), block_type=SpatioTemporalResBlock))
    # model.load_state_dict(torch.load(join(realpath(dirname(__file__)), 'model2/1004model_best.pth')))
    model.eval().cuda()


    dataset = VOTDataset(name='VOT2019',
                        dataset_root='/home/guiyan/workspaces/liangmin/Siam3DM1/Track/data/VOT2019',
                        load_img=True)
